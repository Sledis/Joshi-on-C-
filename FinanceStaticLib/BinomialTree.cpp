...
SimpleBinomialTree::SimpleBinomialTree(double Spot_, const Parameters& r_, const Parameters& d_, double Volatility_, unsigned long Steps_, double Time_) :Spot(Spot_), r(r_), d(d_), Volatility(Volatility_), Steps(Steps_), Time(Time_), Discounts(Steps) {
	TreeBuilt = false;
}
...
double SimpleBinomialTree::GetThePrice(const TreeProduct& TheProduct) {
	...
	for (unsigned long i = 1; i <= Steps; i++) {
		...
		for (long j = -static_cast<long>(index), k = 0; j <= static_cast<long>(index); j = j + 2, k++) {
			...
			//std::cout << TheTree[index][k].second << std::endl;
		}
	}
